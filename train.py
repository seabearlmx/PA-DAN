...
from osdan.dataset.gta5 import GTA5DataSet
from osdan.dataset.cityscapes import CityscapesDataSet
from osdan.domain_adaptation.config import cfg, cfg_from_file
from osdan.model.deeplab_osdan import get_osdan_deeplab
from osdan.domain_adaptation.train_UDA import train_domain_adaptation_with_osdan
...
def main():
    ...
    if cfg.TRAIN.MODEL == 'DeepLabv2_osdan':
        model = get_osdan_deeplab(num_classes=cfg.NUM_CLASSES, multi_level=cfg.TRAIN.MULTI_LEVEL)
        saved_state_dict = torch.load(cfg.TRAIN.RESTORE_FROM, map_location={'cuda:0':'cuda:2'})
    ...
    # UDA TRAINING
    train_domain_adaptation_with_osdan(model, source_loader, target_loader, cfg)
