...
padan_save_path = './result/padan_output'
...
def eval_best(cfg, models,
    ...
    padan_hist = np.zeros((cfg.NUM_CLASSES, cfg.NUM_CLASSES))
    ...
    for index in tqdm(range(len(test_loader))):
        ...
        with torch.no_grad():
            _, pred_padan, pred_main = models[0](image.cuda(device))
            ...
            padan = interp(pred_padan).cpu().data[0].numpy()
            padan = padan.transpose(1, 2, 0)
            padan = np.argmax(padan, axis=2)
            padan_save = np.asarray(padan, dtype=np.uint8)
            padan_col = colorize_mask(padan_save)
            padan_save = Image.fromarray(padan_save)

            name = name[0].split('/')[-1]
            output_save.save('%s/%s' % (output_save_path, name))
            output_col.save('%s/%s_color.png' % (output_save_path, name.split('.')[0]))
            padan_save.save('%s/%s' % (padan_save_path, name))
            padan_col.save('%s/%s_color.png' % (padan_save_path, name.split('.')[0]))

        label = label.numpy()[0]
        hist += fast_hist(label.flatten(), output.flatten(), cfg.NUM_CLASSES)
        padan_hist += fast_hist(label.flatten(), padan.flatten(), cfg.NUM_CLASSES)
        if verbose and index > 0 and index % 100 == 0:
            print('output : {:d} / {:d}: {:0.2f}'.format(
                index, len(test_loader), 100 * np.nanmean(per_class_iu(hist))))
            print('padan: {:d} / {:d}: {:0.2f}'.format(
                index, len(test_loader), 100 * np.nanmean(per_class_iu(padan_hist))))
    inters_over_union_classes = per_class_iu(hist)
    padan_inters_over_union_classes = per_class_iu(padan_hist)
    computed_miou = round(np.nanmean(inters_over_union_classes) * 100, 2)
    padan_computed_miou = round(np.nanmean(padan_inters_over_union_classes) * 100, 2)
    print('\tOutput mIoU:', computed_miou)
    print('\tPadan model:', padan_computed_miou)
    if verbose:
        display_stats(cfg, test_loader.dataset.class_names, inters_over_union_classes)
        display_stats(cfg, test_loader.dataset.class_names, padan_inters_over_union_classes)
